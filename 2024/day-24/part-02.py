...
def execute(input1, input2, func):
    match func:
        case int.__and__:
            return f'''AND({gates[input1]()},{gates[input2]()})'''
        case int.__or__:
            return f'''OR({gates[input1]()},{gates[input2]()})'''
        case int.__xor__:
            return f'''XOR({gates[input1]()},{gates[input2]()})'''
...
if __name__ == '__main__':
    ...
    with open(input_path, 'r') as file:
        ...
        for line in file.readlines():
            gate = re.match(r'^([\w\d]+) (AND|OR|XOR) ([\w\d]+) -> ([\w\d]+)$', line)
            if gate is not None:
                input1, operation, input2, output = gate.groups()
                operations.append((input1, operation, input2, output))
                if output[0] == 'z' and output > highest_z:
                    highest_z = output
        
        wrong_outputs = set()
        for inp1, op, inp2, out in operations:
            if out.startswith('z') and out != highest_z and op != 'XOR':
                wrong_outputs.add(out)
            
            if op == 'XOR' and not out.startswith('z') and not inp1.startswith(('x', 'y')) and not inp2.startswith(('x', 'y')):
                wrong_outputs.add(out)

            if op == 'AND' and 'x00' not in [inp1, inp2]:
                for sub_inp1, sub_op, sub_inp2, sub_out in operations:
                    if out in [sub_inp1, sub_inp2] and sub_op != 'OR':
                        wrong_outputs.add(out)
            
            if op == 'XOR':
                for sub_inp1, sub_op, sub_inp2, sub_out in operations:
                    if out in [sub_inp1, sub_inp2] and sub_op not in ['XOR', 'AND']:
                        wrong_outputs.add(out)
