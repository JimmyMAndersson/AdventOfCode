...
def execute(input1, input2, func):
    return func(gates[input1](), gates[input2]())
...
if __name__ == '__main__':
    ...
    with open(input_path, 'r') as file:
        ...
        for line in file.readlines():
            wire = re.match(r'^([\w\d]+): (\d+)$', line)
            if wire is not None:
                wire, value = wire.groups()
                gates[wire] = partial(lambda x: int(x), value)
            ...
            if gate is not None:
                ...
                match operation:
                    case 'AND':
                        gates[output] = partial(execute, input1, input2, int.__and__)
                    case 'OR':
                        gates[output] = partial(execute, input1, input2, int.__or__)
                    case 'XOR':
                        gates[output] = partial(execute, input1, input2, int.__xor__)
        
        z_wires = sorted(filter(lambda x: x.startswith('z'), gates.keys()))
        results = map(lambda z: gates[z](), z_wires)

        print(sum([(1 << i) * result for i, result in enumerate(results)]))
