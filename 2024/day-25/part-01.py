...
if __name__ == '__main__':
    ...
    with open(input_path, 'r') as file:
        ...
        for schematic in data:
            heights = [list(row) for row in schematic.split('\n')]
            heights = np.array(heights, dtype=str).view('U1').reshape(len(heights), -1)
            heights = np.where(heights == '#', 1, 0).sum(axis=0).reshape(1, -1) - 1
            if schematic[0] == '#':
                locks = np.concatenate((locks, heights), axis=0) if locks is not None else heights
            else:
                keys = np.concatenate((keys, heights), axis=0) if keys is not None else heights
        
        non_overlapping = 0
        for key in keys:
            non_overlapping += locks.shape[0] - np.any((locks + key) >= schematic_height, axis=1).sum()
