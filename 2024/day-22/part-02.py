...
def next_secret(secret: int):
    modulo = 0xFFFFFF
    secret = ((secret << 6) ^ secret) & modulo
    secret = ((secret >> 5) ^ secret) & modulo
    secret = ((secret << 11) ^ secret) & modulo
    return secret

def simulate(n: int, initial_secret: int):
    secret = initial_secret
    previous_bananas = secret % 10
    changes = deque(maxlen=4)
    seen = set()

    for _ in range(n):
        secret = next_secret(secret)
        num_bananas = secret % 10
        changes.append(num_bananas - previous_bananas)
        previous_bananas = num_bananas
        if len(changes) == 4:
            hash = 0
            ...
            seen.add(hash)
            yield (tuple(changes), num_bananas)
...
if __name__ == '__main__':
    ...
    with open(input_path, 'r') as file:
        ...
        for secret in secrets:
            for changes, bananas in simulate(2000, secret):
                log[changes] += bananas
